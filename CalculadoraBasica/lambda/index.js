...
const LaunchRequestHandler = {
    ...
    handle(handlerInput) {
        const speakOutput = 'Bienvenido, abriendo a la calculadora de James Brian';
...
const RaizCuadradaIntentHandler = {
    ...
    handle(handlerInput) {
        var numeroNueve = Number(handlerInput.requestEnvelope.request.intent.slots.numNueve.value);
        var resultado;
        var speakOutput;
        
        if (numeroNueve >= 0) {
            resultado = Math.sqrt(numeroNueve);
            speakOutput = `El resultado de la raíz cuadrada de ${numeroNueve} es ${resultado}`;
        } else {
            speakOutput = 'Lo siento, no puedo calcular la raíz cuadrada de un número negativo. Por favor, indica un número válido.';
        }
...
const ExponenteIntentHandler = {
    canHandle(handlerInput) {
        return Alexa.getRequestType(handlerInput.requestEnvelope) === 'IntentRequest'
            && Alexa.getIntentName(handlerInput.requestEnvelope) === 'ExponenteIntent';
    },
    handle(handlerInput) {
        var base = Number(handlerInput.requestEnvelope.request.intent.slots.numBase.value);
        var exponente = Number(handlerInput.requestEnvelope.request.intent.slots.numExpo.value);
        var resultado;
        var speakOutput;
        
        resultado = Math.pow(base, exponente);
        speakOutput = `El resultado de ${base} elevado a la potencia de ${exponente} es ${resultado}`;
